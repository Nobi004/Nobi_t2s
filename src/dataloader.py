...
class TTSDataset(Dataset):
    def __init__(self, data_dir, max_phoneme_len=100, max_waveform_len=20480, hop_length=256):
        ...
        self.phoneme_to_idx['<PAD>'] = len(self.phoneme_to_idx)  # Add padding token
        
        print(f"Phoneme vocabulary size: {len(self.phoneme_to_idx)}")
        print(f"Phonemes: {list(self.phoneme_to_idx.keys())}")
    def __len__(self):
